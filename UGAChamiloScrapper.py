...
class UGAChamiloScrapper:
    def __init__(
        ...
        self.baseHeaders = _load_json(DEFAULT_HEADERS_PATH) if headers is None else headers
    ...
    def _refresh_modules(self, modulePageUrl):
        ...
        for moduleElement in soup.find_all('h4', attrs={"class": "course-items-title"}):
            ...
            if(not nouveauModule.get_codeName() in ["INFOIA6", "INFOIA7", "INFOIA10", "INFOM3101", "INFOM3101", "INFOM3102", "RIIA03"]):
    ...
    def _get_page(self, url, params=None, check=True):
        ...
        badContentLength = [8987, 7824, 9025]
    ...
    def add_file_download(self, file, path):
        print("adding")
    ...
    def start_download_engine(self, path):
        for module in self.get_moduleList():
            self.downloadEngine.add_file( module.get_rootFolder(), path )
